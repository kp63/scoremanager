...
<!-- 科目別検索フォーム＆結果（test_list.jsp からインクルードされるフラグメント） -->
<div class="mb-5">
	<h3 class="h5 mb-3">科目別検索</h3>
	<form action="TestListSubjectExecute.action" method="get" class="row gx-2 gy-2 align-items-end">
		<div class="col-auto">
			...
			<select id="entYear" name="entYear" class="form-select">
				...
				<c:forEach var="y" items="${entYears}">
					<option value="${y}" <c:if test="${param.entYear == y}">selected</c:if>>${y}年度</option>
				</c:forEach>
			</select>
		</div>
		<div class="col-auto">
			...
			<select id="classNum" name="classNum" class="form-select">
				...
				<c:forEach var="cn" items="${classNums}">
					<option value="${cn}" <c:if test="${param.classNum == cn}">selected</c:if>>${cn}組</option>
				</c:forEach>
			</select>
		</div>
		<div class="col-auto">
...
<c:if test="${not empty subjectResults}">
	<h3 class="h5 mb-2">科目別検索結果: <c:out value="${subjectResults.size()}" /> 件</h3>
		...
		<table class="table table-hover">
			...
			<c:forEach var="item" items="${subjectResults}">
				<tr>
					...
					<td>
						<% Integer p2 = ((bean.TestListSubject) pageContext.getAttribute("item"))
							.getPoints().get(2); %>
						<%= (p2 != null ? p2 : "―") %>
					</td>
				</tr>
...
<c:if test="${subjectNotFound}">
	<div class="text-warning">科目別データが存在しませんでした</div>
</c:if>
